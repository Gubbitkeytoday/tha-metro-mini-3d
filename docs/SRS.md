...
>
> **Updated at MVP 6 (2026-08-02).** Adding MRT Blue — the busiest single line in the feed, 24 trip patterns and 3,712 of the network's 8,193 runs — raised the measured daily peak from 171–172 to **246 concurrent vehicles** (`peak_concurrent`, weekday 2026-07-22 at 19:30; the weekend peak is 212). That is the 300-vehicle gate approached but still not met by real schedule density, and it is the last line the feed has to give: the only remaining in-scope line, MRT Orange, is pre-revenue with no timetable at all. The assertion stays as-is, still failing, for the same reason as before. `MAX_VEHICLES` = 1024 remains ~4× the new peak, so there is no truncation risk.
...
**Delivered in part (2026-08-02).** MRT Blue Line is in the registry and fully simulated (GTFS `route_id` 3, OSM relation 444659), and with it the network reaches **10 lines, 193 stations, 58 trip patterns, 8,193 expanded runs**. Structure is no longer one nominal altitude per line: `tools/track-structure.mjs`'s `deriveStructure()` classifies **each OSM way** by its own `tunnel`/`bridge` tags, so Blue renders as viaduct–tunnel–viaduct (231 elevated / 264 underground track points) and the previously-nominal SRT Red and ARL alignments pick up the short tunnel sections OSM actually records for them. `smoothAltitudes()` then ramps every structure change over ~220 m, so a portal is a gradient rather than a 33 m cliff. Underground transparency (F3.2) is implemented, and day/night lighting (F3.3) is driven by the **simulated clock** through a real solar-position calculation (`src/map/sunPosition.ts`) rather than a manual switch — `auto` places the sun for the scrubbed time, with `day`/`night` available to pin it; the day/night boundary is civil twilight (−6°) and brightness/warmth interpolate across it. Also delivered against MVP 6's polish list: the **shadow quality toggle** (§3A.5; scope is honestly limited — Three can only shadow the track and trains it owns, not MapLibre's building extrusions, which is why it defaults off), the **glassmorphism UI pass**, and **LOD** in the form of the distance fade and screen-space declutter that govern the new floating station-name labels. All of it is exercised by `npm run verify:mvp6` (22 checks). A full responsive/touch pass (phone, landscape phone, tablet, laptop, desktop) ships alongside, covered by `npm run verify:responsive` (54 checks). **Not delivered: the MRT Orange Line.** It has no OpenStreetMap route relation at all — a live Overpass sweep of the Bangkok bbox on 2026-08-02 for `route=subway|train|light_rail|monorail`, for `route=construction`, and for `construction:route`/`proposed:route` returned zero Orange candidates (the only name match in the region is an unrelated `route=ferry` "Orange flag boat", relation 2403874). The repo's pipeline sources every line's geometry from a *pinned* route relation, so there is nothing to pin; adding Orange needs either an upstream OSM relation to appear or a different geometry source, and it is deliberately not faked from memory. See §5's NF1 note for the updated peak-concurrency figure.
...
| 6 | Underground + polish | + MRT Blue (10 lines); Orange **not added** | Yes | F3.2 underground, F3.3, NF2 | Delivered 2026-08-02 except Orange — Blue simulated with real per-way tunnel/viaduct structure; underground transparency, clock-driven sun, shadow toggle, glassmorphism, station labels and a responsive/touch pass all done (`verify:mvp6` 22/22, `verify:responsive` 54/54); Orange blocked on the absence of any OSM route relation, see above |
