...
>
> **MVP 6 update (2026-08-02).** MRT Blue is now built, verified, and delivered — real `route_id "3"`, OSM relation 444659, 10th registry entry, genuinely mixed underground/elevated structure (§7 has the full delivered summary). **MRT Orange and the MRT Purple southern extension ("Purple Phase 2") are still NOT in the registry.** The MVP 6 plan's Task 6 would have added both as track-only, pre-revenue entries (reusing the mechanism this document's §F4.1/§7 MVP 6 describes); that task was **deferred by human ruling** and not built this cycle. The construction-status facts above (Orange late-2027/2030, Purple southern extension 2028/2030) are unchanged — this update only corrects the registry-membership claim, not the underlying transit-authority timelines.
...
**MRT Orange and MRT Purple's southern extension ("Purple Phase 2") were deferred by human ruling (Task 6 of the MVP 6 plan) and are NOT in the registry at all** — not even as track-only. The `preRevenue` rendering mechanism itself (dashed centerline, desaturated deck, registry validation, `LineSelector` badge) was built and unit-tested in an earlier task, but has no real user yet: zero registry lines currently set `preRevenue: true`. Both lines' construction-status facts (§2's caveat block) are unchanged; only the "in the registry" claim needed correcting here. This is the one v1.0 DoD line item this MVP does not fully close — "every operational line simulated" is true (all 10 currently-operational in-scope lines are), but "Orange Line track laid and clearly marked pre-revenue" did not happen this cycle.
...
- **MRT Orange + MRT Purple Phase 2 (track-only, pre-revenue)** — MVP 6's Task 6 (§7) was deferred by human ruling; neither line is in the registry. The `preRevenue` rendering mechanism exists and is unit-tested but has no current user.
- **Manual dark/light theme toggle + basemap style cycling (satellite/terrain)** — scoped and then deferred to a future MVP (MVP 7) after MVP 6's Task 10b (clock-driven basemap theming) shipped. Four hard constraints were established while scoping it, so a future attempt doesn't have to rediscover them: (1) `map.setStyle()` destroys every custom layer including the Three.js scene, so a style cycle must re-run every `style.load` side-effect — the underground-opacity snapshot, the basemap-colour snapshot, vehicle-manager wiring, click handlers, and the sun sync; (2) satellite/terrain are raster basemaps with no `fill`/`fill-extrusion`/`line` layers, so both underground dimming and night theming (both vector-layer-only mechanisms) would silently become no-ops on them unless given raster equivalents (`raster-opacity`/`raster-brightness-*`) or explicitly disabled with that disablement surfaced in the UI; (3) the no-API-key constraint that chose OpenFreeMap Liberty in the first place still applies — vector variants (Liberty/Bright/Positron) stay key-free, but satellite needs a third-party raster source with its own attribution/ToS review, and terrain hillshade needs a public-domain DEM source; (4) a manual light/dark toggle conflicts with Task 10b's clock-driven auto theming, so the right shape is a tri-state Auto/Light/Dark control, not a boolean, with "Auto" preserving today's F3.3 behaviour.
